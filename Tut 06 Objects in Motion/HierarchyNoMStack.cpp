...
{
public:
    Node(glm::vec3 angleParam, glm::vec3 scaleParam, glm::vec3 posParam, bool drawParam)
	: angle(angleParam),
	scale(scaleParam),
	position(posParam),
	drawBool(drawParam){};
    ...
    void renderNode(glm::mat4 const& curr_mat){
		
		glm::mat4 top = curr_mat;
		top *= Translate(position);
		if (angle.x != 0) top *= RotateX(angle.x);
		if (angle.y != 0) top *= RotateY(angle.y);
		if (angle.z != 0) top *= RotateZ(angle.z);
		top *= Scale(scale);
		...
		if (drawBool) {
			glUseProgram(theProgram);
			glBindVertexArray(vao);
			glUniformMatrix4fv(modelToCameraMatrixUnif, 1, GL_FALSE, glm::value_ptr(top));
